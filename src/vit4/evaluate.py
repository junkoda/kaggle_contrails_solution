...
def evaluate(model, loader_val, *, th=0.4):
    ...
    for d in loader_val:
        ...
        if 'y' in d:
            y = d['y'].to(device)          # segmentation mask: 1 x H x W; may be upscaled or annotation mean
            y_sym = d['y_sym'].to(device)
        else:
            y = None

        label = d['label'].to(device)  # ground-truth segmentation mask (always 256 x 256 binary)
        batch_size = len(x)
        ...
        # In validation all are non-augmented (w=1)
        if y is not None:
...
for ifold, (idx_train, idx_val) in enumerate(kfold.split(data.df)):
    ...
    for iepoch in range(epochs):
        ...
        # Epoch done
        if checkpoint and (iepoch + 1) % 10 == 0:
            # Save model
            ofilename = '%s/checkpoint%d_%02d.pytorch' % (odir, ifold, iepoch + 1)
