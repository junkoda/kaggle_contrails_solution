import numpy as np
import glob
import torch
import torch.nn as nn
import torchvision.transforms as T

di = '/kaggle/input/google-research-identify-contrails-reduce-global-warming'


def load_dirs(data_type: str) -> np.ndarray[str]:
    """
    Returns list
      data directories
    """
    dirs = glob.glob('%s/%s/*' % (di, data_type))
    dirs.sort()
    dirs = np.array(dirs)

    return dirs


def load_data(path: str) -> dict:
    """
    Arg:
      path (str): data directory

    Returns: dict
      x (array): (C, 256, 256, T) for C=9 bands and T=8 timesteps
    """
    file_id = path.split('/')[-1]

    # Load input images
    bands = []
    for k in range(8, 17):
        a = np.load('%s/band_%02d.npy' % (path, k))
        bands.append(a)

    x = np.stack(bands, axis=0)  # (C, H, W, T)
    x = x.transpose(3, 0, 1, 2)  # (T, C, H, W)

    ret = {'file_id': file_id,
           'x': x}
    return ret


# Ash-color false color
def rescale_range(x, f_min, f_max):
    # Rescale [f_min, f_max] to [0, 1]
    return (x - f_min) / (f_max - f_min)


def ash_color(x):
    """
    False color for contrail annotation
    x (array): (T, C, H, W) -> (T, 3, H, W)
    """
    r = rescale_range(x[:, 7] - x[:, 6], -4, 2)
    g = rescale_range(x[:, 6] - x[:, 3], -4, 5)
    b = rescale_range(x[:, 6], 243, 303)

    x = np.stack([r, g, b], axis=1)
    x = 1 - x

    return x


class Dataset(torch.utils.data.Dataset):
    """
    d = dataset[i]
    x: image (3, 512, 512) 4 timesteps in one image
    y: segmentation mask (1, H, W)
    """
    def __init__(self, dirs, cfg, *, training=True, augment=False):
        self.dirs = dirs
        nc = cfg['resize']
        self.resize = nn.Identity() if nc == 256 else T.Resize(nc, antialias=False)

    def __len__(self):
        return len(self.dirs)

    def __getitem__(self, i):
        path = self.dirs[i]
        d = load_data(path)
        x = d['x'][1:5]  # (T, C, H, W) with t=1,2,3,4

        # Create color image
        x = ash_color(x)

        x = torch.from_numpy(x)
        x = self.resize(x) # (batch_size, 3, 512, 512])

        # Concatenate 4 images
        #nc = x.size(2)  #  (T, 3, H, W)
        #assert nc in [256, 512]

        #c2 = 2 * nc
        #x = x.numpy()
        #x4 = np.zeros((3, nc2, nc2), dtype=np.float32)

        #x4 = np.zeros((3, 512, 512), dtype=np.float32)
        #x4[:, :nc, :256] = x[3]  # t=4
        #x4[:, :256, 256:] = x[0]
        #x4[:, 256:, :256] = x[1]
        #x4[:, 256:, 256:] = x[2]

        ret = {'file_id': d['file_id'],
               'x': x}  # (T=4, 3, H, W)
        return ret


class Data:
    def __init__(self, data_type, *, debug=False):
        assert data_type in ['train', 'validation', 'test']

        dirs = load_dirs(data_type)
        if debug:
            dirs = dirs[:100]

        self.dirs = dirs

    def __len__(self):
        return len(self.dirs)

    def loader(self, cfg, m):
        batch_size = cfg['batch_size']

        return torch.utils.data.DataLoader(Dataset(self.dirs, m),
                                           batch_size=batch_size,
                                           num_workers=2)
